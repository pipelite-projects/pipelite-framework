...
The `time`, `slf4j`, and `queue` adapters are included in `default-channel-adapters`, already present as a transitive dependency of `pipelite-core`.
...
See xref:testing:index.adoc[Testing] for the full guide.
