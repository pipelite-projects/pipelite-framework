...
A source with no channel adapter scheme is the receiving end of a xref:channel-adapters:link.adoc[`link://`] hop, and its name must be unique within the `PipeliteContext`: `link://order-processing` reaches exactly one flow, and `start()` fails if two declare the same name. It can also declare `?concurrency=N` to process up to `N` messages at the same time instead of one at a time:
