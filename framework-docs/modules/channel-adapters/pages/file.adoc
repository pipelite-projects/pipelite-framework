...
By default, `stateDirectory` resolves under `PipeliteHome` (`~/.pipelite/file-channel-adapter`). Set the `pipelite.home` system property or the `PIPELITE_HOME` environment variable to relocate the state directory for this adapter — and for every other Pipelite component that stores durable state on the local filesystem — without touching `FileChannelConfigurer`. See xref:extensibility:shared-filesystem-storage.adoc[Shared Filesystem Storage] for details.
