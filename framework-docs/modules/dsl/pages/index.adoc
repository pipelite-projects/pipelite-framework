= DSL
:description: The Pipelite Domain Specific Language for declarative flow definition

The Pipelite DSL is a fluent, type-safe Java API that allows integration pipelines to be defined declaratively.
The entry point is always `Pipelite.defineFlow(name)`, which returns a builder with steps constrained by the interface hierarchy.

== Operation hierarchy

[source]
----
FlowOperations
  └─ .fromSource(url)  ──► SourceOperations
                               extends ProcessOperations
                                 ├─ .process(...)
                                 ├─ .transformPayload(...)
                                 ├─ .filter(...)
                                 ├─ .wireTap(...)
                                 ├─ .split(...)
                                 ├─ .toRoute(...)   ──► BuildOperations
                                 └─ .toSink(url)    ──► SinkOperations
                                                          extends BuildOperations

BuildOperations (mutually exclusive - each returns EndOperations, only .build() next)
  ├─ .withRetry(...)
  ├─ .withErrorChannel(...)
  ├─ .withExceptionHandler(...)
  └─ .build()
----

The interface hierarchy ensures that the Java compiler enforces the correct sequence: source → processors → (optional route or sink) → at most one error-handling mode → build.

[cols="1,3"]
|===
|Page |Content

|xref:flow-definition.adoc[Flow Definition]
|All builder operations: `fromSource`, `process`, `transformPayload`, `filter`, `wireTap`, `toSink`, `withRetry`, `withErrorChannel`, `withExceptionHandler`, `build`.

|xref:routing.adoc[Routing]
|Content-based routing, recipient list, and routing slip.

|xref:split-aggregate.adoc[Split / Aggregate]
|Splitting a collection payload into independent messages and aggregating the results.

|xref:expressions.adoc[Expressions]
|Expression syntax used by `filter` and routers.
|===
