...
----
<1> Lists the `@FlowConfiguration` classes to register. Spring constructs them as beans, and the framework automatically scans their `@DefineFlow` methods at startup (see xref:internals:spring-autoconfiguration-internals.adoc[Spring Autoconfiguration Internals] for the exact mechanism).
