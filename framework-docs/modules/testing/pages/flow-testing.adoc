...
----
<1> Real sinks (Kafka, HTTP, …) are transparently redirected to an internal capture endpoint for the duration of the test. The Kafka adapter is never actually instantiated. A `queue://` sink, a hop to another flow, is left as it is.

`flowDefinition(...)` registers the flow(s) under test; `supplyTo(entryPointURL)`, the URL the entry flow declares in `fromSource(...)` (a bare name is rejected), runs the whole pipeline and returns once the exchange has arrived at the sink (or the timeout elapses).

== Flows with no sink

A flow does not need a sink. When it has none, the fixture captures the exchange at the end of its last step, so there is no need for a stand-in `toSink(...)` just to have something to assert on:

[source,java]
----
FlowDefinition flow = Pipelite.defineFlow("normalize-flow")
    .fromSource("queue://raw-in")
    .process("normalize", (io, c) -> io.setOutputPayload(io.getInputPayloadAs(String.class).trim()))
    .build();

given(flowDefinition(flow), inputPayload("  hello "))
    .when(supplyTo("queue://raw-in"))
    .then(isExecutionCompleted(), payloadEquals("hello"));
----

In a chain of flows the exchange is captured where the last one ends.
...
Flows can be chained with a `queue://` sink/source pair and registered together, letting you test a cross-flow pipeline as a single unit:
...
FlowDefinition flowA = Pipelite.defineFlow("flow-a")
    ...
    .toSink("queue://next-entry")
    .build();

FlowDefinition flowB = Pipelite.defineFlow("flow-b")
    .fromSource("queue://next-entry")
