...
* If any element fails during processing, the split step fails as a whole — there is no per-element retry or partial recovery. Combined with `.withRetry(...)` (see xref:core:error-handling.adoc[Error Handling]), a failure causes the entire batch to be retried, not just the failed element.

TIP: A split step runs synchronously and in full within a single node invocation - nothing about it is checkpointed element by element. When it fails, `.withRetry(...)` replays the *entire* split from its original input collection, not just the elements that hadn't finished yet; there is no per-element resume.
...
.split("split-items", segment -> segment
    ...
    .end())
.process("sum-total", (exchange, contribution) -> {
