= Kafka Channel Adapter
:description: Apache Kafka adapter for consumer and producer

The `kafka-channel-adapter` module provides integration with *Apache Kafka 3.6.x*.

== Dependency

[source,xml,subs="attributes+"]
----
<dependency>
    <groupId>{project-group-id}</groupId>
    <artifactId>kafka-channel-adapter</artifactId>
    <version>{project-version}</version>
</dependency>
----

== Usage as source (consumer)

[source,java]
----
Pipelite.defineFlow("kafka-consumer")
    .fromSource("kafka://my-topic")
    .process("handle", exchange -> {
        String record = exchange.getInputPayloadAs(String.class);
        // process the Kafka record
    })
    .toSink("slf4j://log")
    .build();
----

The Kafka source polls the topic on a dedicated background thread.

=== URL format

[source]
----
kafka://<topic-name>[?<params>]
----

== Usage as sink (producer)

[source,java]
----
.toSink("kafka://output-topic")
----

The Kafka sink sends the serialized payload as the Kafka record value.

== Configuration

Kafka configuration is provided via `KafkaChannelConfigurer`:

[source,java]
----
KafkaChannelConfigurer configurer = (config) -> {
    config.setBootstrapServers("localhost:9092");
    config.setGroupId("pipelite-consumer-group");
    config.setAutoOffsetReset("earliest");
};
----

With Spring Boot, register it from a `PipeliteContextInitializer` bean — see xref:spring-integration:bean-configuration.adoc[Configuring Channel Adapters via Beans].

=== Available KafkaChannelConfiguration properties

[cols="1,1,3"]
|===
|Property |Default |Description

|`bootstrapServers`
|`localhost:9092`
|Kafka broker address

|`groupId`
|`pipelite-default`
|Consumer group ID

|`autoOffsetReset`
|`latest`
|Offset reset policy (`earliest` / `latest`)
|===

== Serialization

Java payloads are serialized to/from JSON automatically (via Jackson) when sent to or read from Kafka.
