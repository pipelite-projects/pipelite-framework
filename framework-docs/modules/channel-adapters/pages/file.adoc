...
The same settings can be applied through the typed `FileSourceConfigurer` instead of a query
string (see xref:custom.adoc[Custom Channel Adapters] for the general mechanism):

[source,java]
----
.fromSource("file:///var/log/app.log", (FileSourceConfigurer c) -> c
...
returning a `FileChannelConfigurer` is *not* picked up automatically.
...
|(unset - unrestricted)
|Confines every `file://` sink's target to this directory. `FileProducer` normalizes the resource path and, when this is set, rejects (at flow-registration time) any target that resolves outside it. Unset by default, preserving the historical behavior of accepting any path - this is an opt-in safety net for a context where the resource might be built from external input, not a restriction every caller is forced into.
|===
