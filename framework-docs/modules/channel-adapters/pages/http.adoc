...
The Undertow server starts automatically when the `PipeliteContext` starts, as long as at least one flow has registered an HTTP source.
...
The module also supports sending messages to external HTTP endpoints.
...
The payload is sent as the body of an HTTP POST request.
...
Multiple flows can register different paths on the same underlying HTTP server.
