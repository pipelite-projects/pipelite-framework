...
|`PipeliteContextInitializerBeanPostProcessor`
|Active
|Invokes `initializeContext(PipeliteContext)` on every bean implementing `PipeliteContextInitializer` - a public extension point, not an internal-only one, for anything that needs the context before it starts (e.g. registering a `ChannelConfigurer` - see xref:configuration-properties.adoc[Configuration Properties]).
