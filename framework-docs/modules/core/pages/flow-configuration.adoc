...
|`PipeliteContext.start()` runs a validator chain before starting anything, and throws this listing every problem found: an orphaned `queue://` reference, two flows reading the same queue (see xref:channel-adapters:queue.adoc[Queue Channel Adapter]), or a flow using the `retry-channel` name reserved internally for the built-in retry mechanism.
|===
