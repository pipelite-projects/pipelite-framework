...
/**
 * Terminal node appended internally to a Splitter's inner segment. Captures the Exchange
 * produced by the last segment step for each child, so {@link SplitterNode} can read it back
 * after {@code segmentHead.process(child)} returns.
 *
 * <p>Uses a {@link ThreadLocal} rather than a plain field: {@code FlowNode} instances are built
 * once and reused for every message that flows through the parent {@code SplitterNode}, including
...
final class SplitResultCollectorNode implements FlowNode {
    ...
    public void addExchangePreProcessor(ExchangePreProcessor exchangePreProcessor) {
        // not applicable: internal implementation detail of SplitterNode, never observed externally
    }
