...
The `Expectation` types available in `then(...)` depend on which mode produced the result (see the xref:appendix.adoc[Appendix] for the full breakdown), but the shape of the test — `given` → `when` → `then` — never changes.
