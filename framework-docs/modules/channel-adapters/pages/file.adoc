...
With Spring Boot, register it from a `PipeliteContextInitializer` bean — see xref:spring-integration:bean-configuration.adoc[Configuring Channel Adapters via Beans].
...
|`stateDirectory`
|`PipeliteHome.resolve("file-channel-adapter")` (`~/.pipelite/file-channel-adapter` by default)
...
== Changing the state directory globally

By default, `stateDirectory` resolves under `PipeliteHome` (`~/.pipelite/file-channel-adapter`). Set the `pipelite.home` system property or the `PIPELITE_HOME` environment variable to relocate the state directory for this adapter — and for every other Pipelite component that stores durable state on the local filesystem — without touching `FileChannelConfigurer`. See xref:internals:shared-filesystem-storage.adoc[Shared Filesystem Storage] for implementation details.

== State persistence and resume semantics

Tailing is resumable across restarts: a new `PipeliteContext` pointed at the same `stateDirectory` picks up exactly where the previous process left off, without re-emitting already-consumed content or re-applying `startPosition` semantics. If the source file shrinks below the last known offset (e.g. rotation/truncation), the offset is reset to `0` and a warning is logged.

TIP: For how the state directory's contents are laid out on disk, see xref:internals:file-state-persistence.adoc[File Adapter State Persistence Internals].
