    ...
    public FlowDefinition ingressFlow() {
        return Pipelite.defineFlow("ingress-flow")
            ...
            .process("validate", exchange -> {
                // validation logic — no need to copy the payload to the output:
                // it is forwarded automatically when a processor leaves it untouched
            })
...
For full details on declaring flows — including dependency injection and multiple flow classes — see xref:core:flow-declaration.adoc[Declaring Flows].
...
For a complete, runnable application combining multiple linked flows, concurrent processing, and several channel adapters, see xref:examples:fooddelivery-application.adoc[Food Delivery (Multi-Channel)] in xref:examples:index.adoc[Examples].
